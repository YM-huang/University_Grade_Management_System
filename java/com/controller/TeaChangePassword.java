...
@WebServlet(name = "TeaChangePassword", value = "/TeaChangePassword")
public class TeaChangePassword extends HttpServlet {
    @Override
        ...
        if(oldpassword.equals(teachers.getLpy_Tpassword15())&&newpassword1.equals(newpassword2)){
            ...
            try {
                if(dao.changeTeaPassword(teachers)){
                    ...
                    teachers=dao.findTeaByNo(teachers.getLpy_Tno15());
                    request.getSession().setAttribute("teachers",teachers);
                    RequestDispatcher rd= getServletContext().getRequestDispatcher("/teaChangePassword.jsp");
                    rd.forward(request,response);
                }
